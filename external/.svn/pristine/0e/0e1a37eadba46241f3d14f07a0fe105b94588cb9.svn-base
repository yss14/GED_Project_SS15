...
private:
	...
	static HRESULT createTexture(ID3D11Device* device, const std::string& filename, 
		ID3D11Texture2D** tex, ID3D11ShaderResourceView** srv);
	
private:
	//Filenames
	std::string					filenameT3d;
	std::string					filenameDDSDiffuse;
	std::string					filenameDDSSpecular;
	std::string					filenameDDSGlow;
